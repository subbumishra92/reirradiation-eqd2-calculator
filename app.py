...
# Skin only for 5fx, endpoint ulceration
skin_entry = {
    "OAR":      "Skin",
...
# Append skin to every body entry list
...
with tab2:
    ...
    selected_sites = st.multiselect(
        "Select body site(s)",
        options=list(THREED_CONSTRAINTS.keys())
    )
    ...
    if modality == "3D":
        if not selected_sites:
            ...
            for r in df3d.itertuples():
                ...
                if r.Acceptable:
                    lines.append(f"  Acceptable: {r.Acceptable}")
                lines.append("")
        ...
        if not selected_sites:
            ...
            for r in dfsbrt.itertuples():
                ...
                if r.Optimal is not None:
                    parts.append(f"Opt: {r.Optimal}")
                if r.Mandatory is not None:
                    parts.append(f"Man: {r.Mandatory}")
                if r.Endpoint:
