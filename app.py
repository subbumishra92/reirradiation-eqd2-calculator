...
THREED_CONSTRAINTS = {
    "Head and Neck": [
        ...
        {"OAR":"Skin",           "Preferred":None,                              "Acceptable":"No slices with 100 % circumferential dose; Hot‑spot < 107 %"},
...
# Append skin to all SBRT body lists
...
SBRT_CONSTRAINTS = {
    ...
    "Proximal Upper Extremity": [
        ...
        skin_entry,
        {"OAR":"Cortical Bone","Metric":"V25Gy/V30Gy","3fx_opt":None,"3fx_man":None,"5fx_opt":None,"5fx_man":"V25Gy < 50 %; V30Gy < 35 %","Endpoint":None},
    ...
    "Proximal Lower Extremity": [
        ...
        {"OAR":"Cortical Bone","Metric":"V25Gy/V30Gy","3fx_opt":None,"3fx_man":None,"5fx_opt":None,"5fx_man":"V25Gy < 50 %; V30Gy < 35 %","Endpoint":None},
        {"OAR":"Articular Cartilage","Metric":"Dmax","3fx_opt":None,"3fx_man":None,"5fx_opt":None,"5fx_man":"Dmax ≤ 35 Gy","Endpoint":None},
        {"OAR":"Lumbosacral Plexus","Metric":"D0.03 cc","3fx_opt":None,"3fx_man":None,"5fx_opt":None,"5fx_man":"Dmax ≤ 30 Gy","Endpoint":None}
    ],
    "Distal Lower Extremity": [
        skin_entry,
        {"OAR":"Cortical Bone","Metric":"V25Gy/V30Gy","3fx_opt":None,"3fx_man":None,"5fx_opt":None,"5fx_man":"V25Gy < 50 %; V30Gy < 35 %","Endpoint":None},
    ...
    if st.session_state.stage == "input":
        ...
        for o in selected:
            ...
            for i in range(1, n_courses+1):
                ...
                fx = st.number_input(
                    f"Course {i} fractions", min_value=1, step=1, key=f"{o}_f{i}"
                )
...
with tab2:
    ...
    selected_sites = st.multiselect(
        "Select body site(s)", options=list(THREED_CONSTRAINTS.keys())
    )
    if modality == "3D":
        ...
        if not selected_sites:
            ...
            agg = {}
            for site in selected_sites:
                for row in SBRT_CONSTRAINTS.get(site, []):
                    key = (row["OAR"], row.get("Metric"))
                    if key not in agg:
                        agg[key] = {
                            "OAR":       row["OAR"],
                    ...
                    opt = row.get(f"{fx}fx_opt")
                    man = row.get(f"{fx}fx_man")
                    if opt is not None: agg[key]["Optimal"] = opt
                    if man is not None: agg[key]["Mandatory"] = man
